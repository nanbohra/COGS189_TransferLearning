...
from alignment_methods import euclidean_alignment, riemannian_alignment, srm_alignment
...
if __name__ == "__main__":
    ...
    held_out_subject = (
        args.held_out_subject if args.held_out_subject is not None else n_subjects
    )
    ...
    if args.alignment is not None and args.alignment not in alignment_methods:
        raise ValueError(
            f"unknown alignment method! alignment methods: {list(alignment_methods.keys())}"
        )
    ...
    for subject_id in range(1, n_subjects + 1):
        ...
        print(
            f"Subject {subject_str}: {X.shape}, left fist: {left_count}, right fist: {right_count}"
        )
    ...
    if args.mode == "csp":
        ...
        train_X = np.concatenate(
            [all_X[i] for i in range(1, n_subjects + 1) if i != held_out_subject],
            axis=0,
        )
        train_y = np.concatenate(
            [all_y[i] for i in range(1, n_subjects + 1) if i != held_out_subject],
            axis=0,
        )
        ...
        fig = get_spatial_filter_from_estimator(csp, info=eeg_info).plot_filters(
            components=list(range(csp_components)), show=False
        )
        ...
        for subject_id in range(1, n_subjects + 1):
            ...
            df = pd.DataFrame(
                X_csp, columns=[f"CSP_{i + 1}" for i in range(csp_components)]
            )
            df["label"] = y
            df.to_csv(
                "processed_data/S" + str(subject_id).zfill(3) + ".csv", index=False
            )

        # save combined train data (subjects 1-49)
        train_X_csp = csp.transform(train_X)
        df_train = pd.DataFrame(
            train_X_csp, columns=[f"CSP_{i + 1}" for i in range(csp_components)]
        )
        ...
        df_test = pd.DataFrame(
            test_X_csp, columns=[f"CSP_{i + 1}" for i in range(csp_components)]
        )
        ...
        for subject_id in range(1, n_subjects + 1):
            np.save(
                f"processed_data/eegnet/S{str(subject_id).zfill(3)}_X.npy",
                all_X[subject_id],
            )
            np.save(
                f"processed_data/eegnet/S{str(subject_id).zfill(3)}_y.npy",
                all_y[subject_id],
            )
